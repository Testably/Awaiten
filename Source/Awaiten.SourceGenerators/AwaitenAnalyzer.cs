...
public sealed class AwaitenAnalyzer : DiagnosticAnalyzer
{
	...
	private static List<DiagnosticInfo> Detect(GraphModel graph, bool strict)
	{
		...
		// The keyed-collection membership by service type, so the walk follows a keyed dictionary's members too
		// (a transient disposable keyed member accumulates on the root just like a plain collection member).
		Dictionary<string, List<int>> keyedCollectionMembers =
			AwaitenGenerator.KeyedCollectionMemberIndices(graph.KeyedCollections, graph.ImplToIndex);
		...
		for (int i = 0; i < graph.Instances.Count; i++)
		{
			...
			{
				ReportFromOwner(i, graph, serviceToIndex, collectionMembers, keyedCollectionMembers, strict, reported, diagnostics);
			}
		}
		...
		List<DiagnosticInfo> diagnostics)
	{
		...
		while (stack.Count > 0)
		{
			...
			AddAccumulatingFuncs(node, graph, serviceToIndex, collectionMembers, keyedCollectionMembers, strict, reported, diagnostics);
		...
		List<DiagnosticInfo> diagnostics)
	{
		...
		{
			if (!IsRootAccumulatingFunc(graph, serviceToIndex, collectionMembers, keyedCollectionMembers, parameter) || !reported.Add($"{node}|{parameter.ServiceType}|{parameter.Key}"))
			{
				continue;
			}
	...
	private static bool IsRootAccumulatingFunc(GraphModel graph, Dictionary<ServiceKey, int> serviceToIndex, Dictionary<ServiceKey, List<int>> collectionMembers, Dictionary<string, List<int>> keyedCollectionMembers, ParameterModel parameter)
	{
		...
		return (target.Lifetime == Lifetime.Transient || target.IsParameterized)
		       && AwaitenGenerator.BuildsFreshDisposable(graph.Instances, serviceToIndex, collectionMembers, keyedCollectionMembers, targetIndex);
	}
