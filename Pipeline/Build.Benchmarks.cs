...
partial class Build
{
	Target BenchmarkDotNet => _ => _
		.Executes(() =>
		{
			...
			DotNet(
				$"{Solution.Benchmarks.Awaiten_Benchmarks.Name}.dll --exporters json --filter * --artifacts \"{benchmarkDirectory}\"",
	...
	Target BenchmarkResult => _ => _
		...
		.Executes(async () =>
		{
			string fileContent = await File.ReadAllTextAsync(ArtifactsDirectory / "Benchmarks" / "results" /
			                                                 "Awaiten.Benchmarks.HappyCaseBenchmarks-report-github.md");
		...
		.Executes(async () =>
		{
			...
			if (int.TryParse(prNumber, out int prId))
			{
				...
				IReadOnlyList<IssueComment> comments =
					await gitHubClient.Issue.Comment.GetAllForIssue("vbreuss", "Awaiten", prId);
				long? commentId = null;
				...
				if (commentId == null)
				{
					...
					await gitHubClient.Issue.Comment.Create("vbreuss", "Awaiten", prId, body);
				}
				else
				{
					Log.Information($"Update comment:\n{body}");
					await gitHubClient.Issue.Comment.Update("vbreuss", "Awaiten", commentId.Value, body);
				}
			}
		});
