...
static class BenchmarkReport
{
        ...
        Dictionary<string, string[]> baselineRows, bool injectBaseline, ref bool anyBaselineInjected)
    {
        ...
        foreach (var row in tableBuffer)
        {
            ...
            var isAwaitenRow = row.RowIndex >= BenchmarkTableParser.DataRowStartIndex
                                 && row.Tokens.Length > 0
                                 && row.Tokens[0].Contains("_Awaiten", StringComparison.OrdinalIgnoreCase);

            if (isAwaitenRow && injectBaseline && meanIndex > 0)
            {
    ...
    internal static void RecomputeBaselineRatio(string[] baselineTokens, string[] currentTokens,
        int valueIndex, int ratioIndex)
    {
        if (valueIndex <= 0 || ratioIndex <= 0) return;

        if (valueIndex >= baselineTokens.Length || valueIndex >= currentTokens.Length
                                                || ratioIndex >= baselineTokens.Length)
            return;

        if (!TryParseQuantity(baselineTokens[valueIndex], out var baselineValue)
            || !TryParseQuantity(currentTokens[valueIndex], out var currentValue)
            || currentValue <= 0)
            return;

        baselineTokens[ratioIndex] = (baselineValue / currentValue).ToString("F2", CultureInfo.InvariantCulture);
    }
